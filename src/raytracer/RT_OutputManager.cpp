...
void RT_OutputManager::export_picture(std::string name) {
    ...
    } else if (config.rtMode == RT_RayRenderMode::RT_DEPTHMAP_BW) {
        ...
        for (unsigned y = 0; y < height; y++) {
            ...
            if (rayOutput.distance >= 0) { // If the ray hits an object
                if (config.rtMode == RT_RayRenderMode::RT_BITMAP) {
                    ...
                    pic.writePixel(Color(1), rayOutput.ortho_distance, x, y);
                }
            } else {
                if (config.rtMode == RT_RayRenderMode::RT_DEPTHMAP_BW) {
                    pic.writePixel(Color(0.0), x, y);
                } else if (config.env->show_background_color) {
