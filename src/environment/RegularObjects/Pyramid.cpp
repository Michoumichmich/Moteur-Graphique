...
void Pyramid::Tesselate(int resolution) {
    double center_dist = (edge/2) * tan(M_PIl/6);
    Point3D pt1(center_dist, -edge/2, 0);
    Point3D pt2(center_dist, edge/2, 0);
    Point3D pt3(-(edge*sqrt(5)/2 - center_dist), 0, 0);
    Point3D pt4(0, 0, edge);
